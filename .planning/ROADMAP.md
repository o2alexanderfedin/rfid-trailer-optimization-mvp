...
> **Keystone constraint:** determinism. HOS-*off* stays byte-identical to the pre-v1.2 golden replay; HOS-*on* adds a new golden. All HOS RNG flows through one new isolated seeded substream. Grounding: `research/v1.2-DRIVER-HOS-GROUNDING.md`, `research/v1.2-HUB-DETAIL-GROUNDING.md`. Build-order: domain → HOS engine → deterministic sim → projection → endpoint → optimizer (aware → enforced) → UI → docs.

**Phase 9: Driver domain model + HOS config + duty/phase events**
...
**Phase 11: Simulation HOS enforcement + load/unload events + determinism golden**
...
**Phase 14: Hub-detail read endpoint + ws driver buckets**
